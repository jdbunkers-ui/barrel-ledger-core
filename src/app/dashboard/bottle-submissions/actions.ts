...
export async function submitBottleAction(formData: FormData) {
  ...
  const email = val(formData, "email_address");
  ...
  if (!existingBottleId && (!val(formData, "brand_name") || !val(formData, "spirit_category") || !val(formData, "expression_name"))) {
    throw new Error("Select an existing bottle or provide brand, category, and expression for a new bottle.");
  }
  ...
  const { error: distilleryError } = await supabase
    ...
    .insert({
      submission_id,
      existing_distillery_id: existingDistilleryId,
      distillery_name: val(formData, "distillery_name"),
  ...
  const { error: bottleError } = await supabase
    ...
    .insert({
      ...
      brand_name: val(formData, "brand_name"),
      spirit_category: val(formData, "spirit_category"),
      spirit_subtype: val(formData, "spirit_subtype"),
      expression_name: val(formData, "expression_name"),
  ...
  if (barrelType) {
    const { error: sbError } = await supabase
      ...
      .insert({
        ...
        pick_name: val(formData, "pick_name"),
        ...
        age_statement_total_months: intVal(formData, "age_statement_total_months"),
        distilled_state: val(formData, "distilled_state"),
        bottled_state: val(formData, "bottled_state"),
        single_barrel_description: val(formData, "single_barrel_description"),
  ...
  if (existingPickerId || val(formData, "barrel_picker_name")) {
    const { error: pickerError } = await supabase
      ...
      .insert({
        submission_id,
        existing_barrel_picker_id: existingPickerId,
        barrel_picker_name: val(formData, "barrel_picker_name"),
